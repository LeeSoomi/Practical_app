...
def _asset_candidates(relative_under_assets: str) -> list[Path]:
    """Streamlit Cloud 등에서 작업 디렉터리·폴더 구조가 달라질 수 있어 후보 경로를 순서대로 검사한다."""
    rel = Path(relative_under_assets)
    cwd = Path.cwd().resolve()
    return [
        _BASE / "assets" / rel,
        cwd / "assets" / rel,
        cwd / "zone3" / "assets" / rel,
        _BASE.parent / "zone3" / "assets" / rel,
    ]
...
def _scenario_image_path(question: dict) -> Path | None:
    ...
    if not rel:
        return None
    for p in _asset_candidates(rel):
        if p.is_file():
            return p
    return None


def _read_svg_as_text(path: Path) -> str:
    """BOM·UTF-16 등 환경 차이로 read_text만 쓰면 실패하는 경우를 줄인다."""
...
def _render_situation_image(path: Path, caption: str | None, *, iframe_height: int) -> None:
    """SVG는 st.image가 잘못된 인코딩으로 열어 깨지는 경우가 많아 iframe에 인라인 삽입한다."""
    if path.suffix.lower() == ".svg":
        try:
            svg = _read_svg_as_text(path)
        except OSError as e:
            st.warning("상황 일러스트(SVG) 파일을 열 수 없습니다. assets 폴더가 배포에 포함됐는지 확인하세요.")
            st.caption(str(e))
            return
        if not svg.strip() or "<svg" not in svg.lower():
            st.warning("SVG 내용이 비어 있거나 형식이 올바르지 않습니다.")
            return
        svg = re.sub(
            r"<svg\s+",
...
# ─────────────────────────────────────────────
    ...
    if st.session_state.step == 2:
        ...
        if img_path is not None:
            _render_situation_image(
                img_path,
                f"{q['image_desc']} · 교육용 상황 일러스트 (실제 영상·기록 아님)",
                iframe_height=340,
            )
        st.markdown(
            f"""
        ...
        if img_path is not None:
            ...
            with img_col:
                _render_situation_image(img_path, "상황 참고", iframe_height=300)
            with q_col:
